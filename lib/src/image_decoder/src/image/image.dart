...
class Image extends Iterable<Pixel> {
  ...
  Image.fromResized(Image other,
    ...
    if (!noAnimation) {
      final int numFrames = other.numFrames;
      for (int fi = 1; fi < numFrames; ++fi) {
        final Image frame = other.frames[fi];
  ...
  Image.fromBytes(
    ...
    final Uint8List toBytes = data!.toUint8List();
    final Uint8List fromBytes = Uint8List.view(bytes, bytesOffset);

    rowStride ??= getRowStride(width, numChannels, format);
    final int dataStride = data!.rowStride;
    final int stride = min(rowStride, dataStride);

    int dOff = 0;
    int bOff = 0;
    for (int y = 0; y < height; ++y, bOff += rowStride, dOff += dataStride) {
      final Uint8List bRow = Uint8List.sublistView(fromBytes, bOff, bOff + stride);
      toBytes.setRange(dOff, dOff + dataStride, bRow);
    }

    if (numChannels == 3 && order == ChannelOrder.bgr) {
      for (final Pixel p in this) {
        final num r = p.r;
        p
          ..r = p.b
          ..b = r;
  ...
  void setExtraChannel(String name, ImageData? data) {
    ...
    extraChannels ??= <String, ImageData>{};
  ...
  Color getPixelLinear(num fx, num fy) {
    final int x = fx.toInt() - (fx >= 0 ? 0 : 1);
    final int nx = x + 1;
    final int y = fy.toInt() - (fy >= 0 ? 0 : 1);
    final int ny = y + 1;
    final num dx = fx - x;
    final num dy = fy - y;
    ...
    final Pixel icc = getPixelSafe(x, y);
    final Pixel icn = ny >= height ? icc : getPixelSafe(x, ny);
    final Pixel inc = nx >= width ? icc : getPixelSafe(nx, y);
    final Pixel inn = nx >= width || ny >= height ? icc : getPixelSafe(nx, ny);
  ...
  Color getPixelCubic(num fx, num fy) {
    final int x = fx.toInt() - (fx >= 0.0 ? 0 : 1);
    final int px = x - 1;
    final int nx = x + 1;
    final int ax = x + 2;
    final int y = fy.toInt() - (fy >= 0.0 ? 0 : 1);
    final int py = y - 1;
    final int ny = y + 1;
    final int ay = y + 2;
    ...
    final Pixel ipp = px < 0 || py < 0 ? icc : getPixelSafe(px, py);
    final Pixel icp = px < 0 ? icc : getPixelSafe(x, py);
    final Pixel inp = py < 0 || nx >= width ? icc : getPixelSafe(nx, py);
    final Pixel iap = ax >= width || py < 0 ? icc : getPixelSafe(ax, py);

    final num ip0 = cubic(dx, ipp.r, icp.r, inp.r, iap.r);
    final num ip1 = cubic(dx, ipp.g, icp.g, inp.g, iap.g);
    final num ip2 = cubic(dx, ipp.b, icp.b, inp.b, iap.b);
    final num ip3 = cubic(dx, ipp.a, icp.a, inp.a, iap.a);

    final Pixel ipc = px < 0 ? icc : getPixelSafe(px, y);
    final Pixel inc = nx >= width ? icc : getPixelSafe(nx, y);
    final Pixel iac = ax >= width ? icc : getPixelSafe(ax, y);

    final num ic0 = cubic(dx, ipc.r, icc.r, inc.r, iac.r);
    final num ic1 = cubic(dx, ipc.g, icc.g, inc.g, iac.g);
    final num ic2 = cubic(dx, ipc.b, icc.b, inc.b, iac.b);
    final num ic3 = cubic(dx, ipc.a, icc.a, inc.a, iac.a);

    final Pixel ipn = px < 0 || ny >= height ? icc : getPixelSafe(px, ny);
    final Pixel icn = ny >= height ? icc : getPixelSafe(x, ny);
    final Pixel inn = nx >= width || ny >= height ? icc : getPixelSafe(nx, ny);
    final Pixel ian = ax >= width || ny >= height ? icc : getPixelSafe(ax, ny);

    final num in0 = cubic(dx, ipn.r, icn.r, inn.r, ian.r);
    final num in1 = cubic(dx, ipn.g, icn.g, inn.g, ian.g);
    final num in2 = cubic(dx, ipn.b, icn.b, inn.b, ian.b);
    final num in3 = cubic(dx, ipn.a, icn.a, inn.a, ian.a);

    final Pixel ipa = px < 0 || ay >= height ? icc : getPixelSafe(px, ay);
    final Pixel ica = ay >= height ? icc : getPixelSafe(x, ay);
    final Pixel ina = nx >= width || ay >= height ? icc : getPixelSafe(nx, ay);
    final Pixel iaa = ax >= width || ay >= height ? icc : getPixelSafe(ax, ay);

    final num ia0 = cubic(dx, ipa.r, ica.r, ina.r, iaa.r);
    final num ia1 = cubic(dx, ipa.g, ica.g, ina.g, iaa.g);
    final num ia2 = cubic(dx, ipa.b, ica.b, ina.b, iaa.b);
    final num ia3 = cubic(dx, ipa.a, ica.a, ina.a, iaa.a);

    final num c0 = cubic(dy, ip0, ic0, in0, ia0);
    final num c1 = cubic(dy, ip1, ic1, in1, ia1);
    final num c2 = cubic(dy, ip2, ic2, in2, ia2);
    final num c3 = cubic(dy, ip3, ic3, in3, ia3);
  ...
  Image convert(
    ...
    Image? firstFrame;
    for (final Image frame in frames) {
      ...
      final Palette? pal = newImage.palette;
      final Format f = newImage.palette?.format ?? format;
      if (pal != null) {
        final Map<int, int> usedColors = <int, int>{};
        int numColors = 0;
        final Pixel op = frame.getPixel(0, 0);
        Color? c;
        for (final Pixel np in newImage) {
          final int nr = (op.rNormalized * 255).floor();
          final int ng = (op.gNormalized * 255).floor();
          final int nb = (op.bNormalized * 255).floor();
          final int h = rgbaToUint32(nr, ng, nb, 0);
