...
Color _convertColor(Color c, Color c2, num a) {
  final int numChannels = c2.length;
  final Format format = c2.format;
  final Format fromFormat = c.palette?.format ?? c.format;
  final int cl = c.length;
  if (numChannels == 1) {
    final num g = c.length > 2 ? c.luminance : c[0];
    final num gi = (c[0] is int) ? g.floor() : g;
    ...
    if (cl == 2) {
      final num l = convertFormatValue(c[0], fromFormat, format);
      if (numChannels == 3) {
        c2[0] = l;
        c2[1] = l;
        c2[2] = l;
      } else {
        final num a = convertFormatValue(c[1], fromFormat, format);
        c2[0] = l;
        c2[1] = l;
        c2[2] = l;
        c2[3] = a;
      ...
      final num v = cl == 1 ? c2[0] : 0;
      for (int ci = cl; ci < numChannels; ++ci) {
        c2[ci] = ci == 3 ? a : v;
      }
    }
  }
...
Color convertColor(Color c,
  ...
  switch (format) {
    case Format.uint8:
      final Color c2 = to ?? ColorUint8(numChannels);
      return _convertColor(c, c2, alpha);
    case Format.uint1:
      final Color c2 = to ?? ColorUint1(numChannels);
      return _convertColor(c, c2, alpha);
    case Format.uint2:
      final Color c2 = to ?? ColorUint2(numChannels);
      return _convertColor(c, c2, alpha);
    case Format.uint4:
      final Color c2 = to ?? ColorUint4(numChannels);
      return _convertColor(c, c2, alpha);
    case Format.uint16:
      final Color c2 = to ?? ColorUint16(numChannels);
      return _convertColor(c, c2, alpha);
    case Format.uint32:
      final Color c2 = to ?? ColorUint32(numChannels);
      return _convertColor(c, c2, alpha);
    case Format.int8:
      final Color c2 = to ?? ColorInt8(numChannels);
      return _convertColor(c, c2, alpha);
    case Format.int16:
      final Color c2 = to ?? ColorInt16(numChannels);
      return _convertColor(c, c2, alpha);
    case Format.int32:
      final Color c2 = to ?? ColorInt32(numChannels);
      return _convertColor(c, c2, alpha);
    case Format.float16:
      final Color c2 = to ?? ColorFloat16(numChannels);
      return _convertColor(c, c2, alpha);
    case Format.float32:
      final Color c2 = to ?? ColorFloat32(numChannels);
      return _convertColor(c, c2, alpha);
    case Format.float64:
      final Color c2 = to ?? ColorFloat64(numChannels);
      return _convertColor(c, c2, alpha);
  }
}
...
List<int> hslToRgb(num hue, num saturation, num lightness) {
  if (saturation == 0) {
    final int gray = (lightness * 255.0).toInt();
    return <int>[gray, gray, gray];
  }
  ...
  final num q = lightness < 0.5
      ? lightness * (1.0 + saturation)
      : lightness + saturation - lightness * saturation;
  final double p = 2.0 * lightness - q;

  final num r = hue2rgb(p, q, hue + 1.0 / 3.0);
  final num g = hue2rgb(p, q, hue);
  final num b = hue2rgb(p, q, hue - 1.0 / 3.0);

  return <int>[(r * 255.0).round(), (g * 255.0).round(), (b * 255.0).round()];
}
...
List<int> hsvToRgb(num hue, num saturation, num brightness) {
  if (saturation == 0) {
    final int gray = (brightness * 255.0).round();
    return <int>[gray, gray, gray];
  }

  final num h = (hue - hue.floor()) * 6.0;
  final num f = h - h.floor();
...
List<num> rgbToHsl(num r, num g, num b) {
  ...
  g /= 255.0;
  b /= 255.0;
  final num mx = max(r, max(g, b));
  final num mn = min(r, min(g, b));
  num h;
  final double l = (mx + mn) / 2.0;

  if (mx == mn) {
    return <num>[0.0, 0.0, l];
  }

  final num d = mx - mn;

  final double s = l > 0.5 ? d / (2.0 - mx - mn) : d / (mx + mn);
...
List<int> labToXyz(num l, num a, num b) {
  ...
  return <int>[(x * 95.047).toInt(), (y * 100.0).toInt(), (z * 108.883).toInt()];
}
...
List<num> xyzToLab(num x, num y, num z) {
  ...
  return <num>[(116.0 * y) - 16, 500.0 * (x - y), 200.0 * (y - z)];
}
