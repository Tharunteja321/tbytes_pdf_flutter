...
class OutputBuffer {
  ...
  void writeBuffer(InputBuffer bytes) {
    ...
    final int requiredLength = length + bytesLength;
  ...
  void writeFloat32(double value) {
    final Float32List fb = Float32List(1);
    fb[0] = value;
    final Uint8List b = Uint8List.view(fb.buffer);
    if (bigEndian) {
      writeByte(b[3]);
      writeByte(b[2]);
      writeByte(b[1]);
  ...
  void writeFloat64(double value) {
    final Float64List fb = Float64List(1);
    fb[0] = value;
    final Uint8List b = Uint8List.view(fb.buffer);
    if (bigEndian) {
      writeByte(b[7]);
  ...
  void _expandBuffer([int? required]) {
    final int blockSize = (required != null)
        ...
        : (_buffer.isEmpty)
            ? _blockSize
            : (_buffer.length * 2);
    final Uint8List newBuffer = Uint8List(_buffer.length + blockSize)
      ..setRange(0, _buffer.length, _buffer);
    _buffer = newBuffer;
  }

  static const int _blockSize = 0x2000; // 8k block-size
  Uint8List _buffer;
}
